...
export default function AdminDashboard() {
  ...
  const fetchAnalytics = async () => {
    try {
      const res = await axios.get("https://zoya-backend.onrender.com/admin/analytics", {
  ...
  const fetchUsers = async () => {
    try {
      const res = await axios.get("https://zoya-backend.onrender.com/admin/users", {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.data.ok) {
  ...
  const fetchVendors = async () => {
    try {
      const res = await axios.get("https://zoya-backend.onrender.com/admin/vendors", {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.data.ok) {
  ...
  return (
    <div className="bg-gray-900 min-h-screen font-sans text-white p-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 space-y-4 md:space-y-0">
        <h1 className="text-4xl font-bold text-purple-400 tracking-wide">Zoya Admin Dashboard</h1>
      ...
      <section className="mb-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { label: "Total Users", value: analytics.totalUsers || 0, color: "from-purple-600 to-purple-400" },
            { label: "Total Vendors", value: analytics.totalVendors || 0, color: "from-green-500 to-green-400" },
            { label: "Waitlist Signups", value: analytics.waitlistSignups || 0, color: "from-purple-500 to-purple-300" },
          ].map((card) => (
            <div
              key={card.label}
      ...
      <section className="mb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredUsers.map((u) => (
            <div key={u.id} className="bg-gray-800 p-4 rounded-xl shadow-lg hover:shadow-2xl transition transform hover:scale-105">
              ...
              <p className="text-gray-500 text-xs">Joined: {new Date(u.created_at).toLocaleDateString()}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredVendors.map((v) => (
            <div key={v.id} className="bg-gray-800 p-4 rounded-xl shadow-lg hover:shadow-2xl transition transform hover:scale-105">
              ...
              <p className="text-gray-500 text-xs">Joined: {new Date(v.created_at).toLocaleDateString()}</p>
            </div>
          ))}
        </div>
      </section>
